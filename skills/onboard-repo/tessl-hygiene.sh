...
#   1. Pin every gamussa/*-owned dependency in tessl.json to "latest" (stops the
...
#
# Idempotent: a gamussa/* dep already at "latest" is left untouched; the block is
...
MARKER="=== Tessl-generated artifacts (managed by gamussa/coding-policy) ==="

gitignore_block() {
  cat <<'EOF'
# === Tessl-generated artifacts (managed by gamussa/coding-policy) ===
...
main() {
  ...
  # 1. Pin gamussa/* deps in tessl.json to latest.
  local tj_state="absent"
  if [[ -f tessl.json ]]; then
    local pinned
    pinned=$(jq -r '[.dependencies // {} | to_entries[] | select((.key | startswith("gamussa/")) and .value.version != "latest")] | length' tessl.json) \
    ...
    if [[ "$pinned" == "0" ]]; then
      ...
      if ! jq --indent 2 '.dependencies |= with_entries(if (.key | startswith("gamussa/")) then .value.version = "latest" else . end)' tessl.json > "$tmp"; then
