...
- The Stop hook blocks the handoff on the engine's error and warning tiers only; info and style tiers (shellcheck `info`/`style`) are reported on stderr, and the author fixes or inline-suppresses them before the CI gate, which stays at zero
